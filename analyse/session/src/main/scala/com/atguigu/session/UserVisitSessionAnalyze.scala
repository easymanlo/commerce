...
object UserVisitSessionAnalyze {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 将数据转换为Session粒度，一个完整的信息， 格式为<sessionid,(sessionid,searchKeywords,clickCategoryIds,age,professional,city,sex)>
    //一个sessionid对应一个条聚合处理后的数据
    val sessionid2AggrInfoRDD: RDD[(String, String)] = aggregateBySession(spark, sessionid2actionRDD)
  ...
  /**
    ...
    * @param taskid
    */
  def getTop10Session(spark: SparkSession,
                      taskid: String,
    ...
    // 获得所有需要求的category集合   把符合top10商品品类的过滤出来
    ...
    // 获取每个品类被每一个Session点击的次数  categoryid2sessionCountRDD（categoryid, sessionid + "," + count）
  ...
  def getOrderCategoryId2CountRDD(sessionid2detailRDD: RDD[(String, UserVisitAction)]): RDD[(Long, Long)] = {
    ...
    // 获取每种类别的下单次数 userVisitAction.order_category_ids.split(","):Array[String]
    //然后使用map转化数组中的每一个元素
    //每一个元素都是一个String 要把String转化成long 这个  (item.toLong, 1L)) }
  ...
  def joinCategoryAndData(categoryidRDD: RDD[(Long, Long)],
    ...
    // 将所有品类信息与点击次数信息结合【左连接】  左连接就是保留左边的信息，就是过滤
    val clickJoinRDD = categoryidRDD.leftOuterJoin(clickCategoryId2CountRDD).map { case (categoryid, (cid, optionValue)) =>
      //optionValue.isDefined 这个就是判断option中是否有数据，要是有就是ture 反之  如果是 ture   optionValue.get
      //这个可以获取option中的数据 option就是防止空值异常   some  none
  ...
  def getTop10Category(spark: SparkSession,
    ...
    // 获取所有产生过点击、下单、支付中任意行为的商品类别  可以把userVisitAction 中的集合也打碎，变成一个长串
    //这个 categoryidRDD 可能重复，下一步进行去重
    val categoryidRDD: RDD[(Long, Long)] = sessionid2detailRDD.flatMap { case (sessionid, userVisitAction) =>
      ...
      // 这快就相当于很多函数进行一个运算后，生成了一个list，然后再对list做了一个flatmap
      list
    }
    ...
    val sortKey2countRDD = categoryid2countRDD.map { case (categoryid, line) =>
      ...
      //这个是返回的一个自定义的一个key排序  这个key已经是一个可排序的key
    ...
    // 第六步：用take(10)取出top10热门品类，并写入MySQL take 返回的是一个array类型的
    val top10CategoryList = sortedCategoryCountRDD.take(10)
     //这个快也可以先转化成rdd   spark.sparkContext.makeRDD(top10CategoryList)
    ...
    val top10Category : Array[Top10Category] = top10CategoryList.map { case (categorySortKey, line) =>
      ...
      val orderCount = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_ORDER_COUNT).toLong
      val payCount = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_PAY_COUNT).toLong
      //返回一个样例类 不用new  1.可以进行模式匹配
      //2.自动生成半生对象，实现了apply方法，再创建对象时候不用new
      //3.实现了unapply方法，可以方便进行模式匹配
      //4.实现了，equles，hashcode，copy，等方法。
  ...
  def randomExtractSession(spark: SparkSession,
    ...
    // countMap<yyyy-MM-dd_HH, count>  这个就知道了每个小时的的sessionId数量
    //知道了每一个小时的sessionid的数量
    ...
    for ((dateHour, count) <- countMap) {
      ...
      // 通过模式匹配实现了if的功能   （get是一个option类型  这个下面有二个子类  一个是some包含了那个返回值 一个是none 为了防止空值异常）
      //get（date）就是获取value
      //map.size 就是key的个数 ，
    ...
    val sessionRandomExtract = time2sessionsRDD.flatMap { case (dateHour, items) =>
      ...
      // 开始遍历所有的aggrInfo     (session ，aggrInfo)
  ...
  /**
    ...
    * @return
    */
  def aggregateBySession(spark: SparkSession,
                         sessinoid2actionRDD: RDD[(String, UserVisitAction)]): RDD[(String, String)] = {
    ...
    val userid2PartAggrInfoRDD = sessionid2ActionsRDD.map { case (sessionid, userVisitActions) =>
      ...
      //session访问步长，就是一个sessio中action的个数
  ...
  def getActionRDDByDateRange(spark: SparkSession, taskParam: JSONObject): RDD[UserVisitAction] = {
    ...
    import spark.implicits._
    val rdd: RDD[UserVisitAction] = spark.sql("select * from user_visit_action where date>='" + startDate + "' and date<='" + endDate + "'")
